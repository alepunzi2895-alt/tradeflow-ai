...
**ai_review pattern**: ogni chiamata Python a Codex DEVE passare da `scripts/ai_review.py` (`call_claude()`/`call_claude_json()`) — mai `requests.post` diretto duplicato nei singoli script.
...
scripts/
  ...
  ai_review.py       — client condiviso per chiamate Codex (Python)
