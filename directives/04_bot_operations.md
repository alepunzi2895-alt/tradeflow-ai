...
| UI | **Dashboard**: `#layout-strategy-cards` — 4 card `mfkk-card` (S31/S32/S33/S34), full-width con griglia propria a 2 colonne (`style.css`, tablet+desktop; 1 colonna sotto 768px), una per strategia, look identico a "MFKK Strategy Score": pill **BUY/SELL** (`.mfkk-dir`, evidenziato in automatico secondo `su.bias`, non interattivo) + ring (confidence 0-100 o score setup) + bias/fase + **righe indicatori cliccabili** (`layout_confidence.indicator_readout` — value + barra bull/bear + score −100..+100 + stato; click → stesso `#info-overlay` della card MFKK con titolo/spiegazione da `LAYOUT_INDICATOR_DEFS`, 19 voci in `dashboard.js`) + chip fattori confidence + footer (P&L live per S31, "solo score" per le altre). `loadLayoutStrategies()`/`renderLayoutStratCard()`/`showLayoutIndicatorInfo()` in `dashboard.js` (righe con `data-*` + `addEventListener` delegato, non `onclick` inline — vedi `08_dev_rules.md`). **Tab Strategie**: card badge `🔬 SOLO SCORE` + `researchNote` |
