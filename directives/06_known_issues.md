...
| ✅ Fixed 2026-09-02 (worktree sprint) | `scripts/mt5-bot.py` | **Bot fermo da trade dal 2026-07-10 — `_strategy_order_tickets` si blocca a `MAX_OPEN_ORDERS`**: alla chiusura di una posizione, `strategy_closed` veniva derivato dal comment MT5 troncato (es. `S18_RANGE_` invece di `S18_RANGE_REVERSAL`) → `_strategy_order_tickets.pop(strategy_closed, None)` non matchava mai, l'entry restava agganciata per sempre. `count_open_positions()` = `max(mt5_count, len(_strategy_order_tickets))` non scendeva mai sotto le stale entries → tutti i blocchi segnale controllano `count_open_positions() >= MAX_OPEN_ORDERS` **prima** del self-heal in `has_open_position_for_strategy()` → deadlock permanente. **Fix parziale già presente** (commit d6c89a9): `strategy_closed` risolto per ticket-match contro `_strategy_order_tickets`. **Fix completato 2026-09-02**: riconciliazione attiva nel sync loop — a ogni sync, ogni entry in `_strategy_order_tickets` il cui ticket non è tra le posizioni MT5 reali viene rimossa (due-strike: solo dopo 2 sync consecutivi stale, per non togliere posizioni appena aperte non ancora visibili in MT5). Idem `_strategy_sl_count`. `_stale_strikes` set locale a `run()`. **Serve comunque un restart del bot sulla VPS** per svuotare lo stato in-memory corrente. |
