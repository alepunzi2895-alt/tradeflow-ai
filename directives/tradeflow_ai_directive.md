...
| 2026-04-16 | Ricalcolo performance post-ottimizzazione | Dopo il rilassamento filtri, le statistiche in dashboard erano obsolete. | Eseguito `backtest_combined.py` con nuova logica: P&L 24m salito a +$22.042, PF globale 2.06, WinRate 42%. Aggiornata UI Dashboard con nuovi KPI reali. |
| 2026-04-16 | `api/report.js` Anthropic fetch senza timeout | Nessun `fetchT` helper definito nel file → chiamata Anthropic poteva bloccare la Vercel function per 10s causando timeout 504 | Aggiunto `fetchT` helper all'inizio di `report.js`, chiamata Anthropic avvolta con timeout 9s |
| 2026-04-16 | `api/price.js` usava `GC=F` come fallback candele | Candles endpoint (type=candles) aveva `['XAUUSD=X', 'GC=F']` — GC=F futures ≠ spot, introduceva drift negli indicatori CCI/MACD/ADX | Rimosso `GC=F` dal fallback; solo `XAUUSD=X` per XAU; solo `XAGUSD=X` per XAG |
| 2026-04-16 | Bot MT5 reconnect senza exponential backoff | Riconnessione usava wait fisso 5s+30s ripetuto identico → se MT5 era stabile down, bot spammava reconnect ogni 35s senza aumentare attesa | Aggiunto `reconnect_attempts` counter; exponential backoff: 5s → 10s → 20s → 40s → ... max 300s; reset a 0 dopo riconnessione riuscita |
| 2026-04-16 | CLAUDE.md mancava S10 e S16 nella tabella Active Strategies | Le strategie S10_OB_FVG_SCALP e S16_GOLDEN_SQUEEZE erano attive nel bot (STRATEGY_PARAMS) ma non documentate in CLAUDE.md | Aggiornata sezione Active Strategies con tabella Bot MT5 (S05/S09/S10/S16) separata dalla tabella UI; aggiunta sezione Known Issues |
| 2026-04-16 | README.md obsoleto | Referenziava file inesistenti (api/indicators.js, api/kb.js, api/cot-update.js, api/market.js come primario) | Riscritto completamente: struttura corrente, env vars completo, comandi bot/backtest |
| 2026-04-16 | api/webhook.js: memCache senza TTL | Cache in-memory non aveva scadenza → stale indicators dopo assenza webhook | Aggiunto CACHE_TTL_MS = 5min, timestamp memCacheTs; cache scade e fallback su GitHub |
| 2026-04-16 | api/webhook.js: nessuna firma TradingView | Chiunque poteva inviare fake segnali al webhook | Aggiunta verifica opzionale header X-TV-Secret vs env var TV_WEBHOOK_SECRET (backward compatible) |
| 2026-04-16 | mt5-bot.py: get_candles(300) ogni 10s | Fetch 300 candele MT5 ogni CHECK_SEC=10s → 360 chiamate/ora anche senza nuova barra | Aggiunta cache 60s: now_ts - last_candle_fetch_ts >= 60 prima di ri-fetchare |
| 2026-04-16 | mt5-bot.py: compute_indicators() ogni 10s | Tutti gli indicatori ricalcolati ogni tick anche su dati invariati → CPU waste | Cache cached_I_h1: ricalcolo solo su new_h1_bar (1×/ora per H1); manage_positions() usa ATR cached |
| 2026-04-16 | strategy-engine-v2.py: S16 TP/SL errato in run_one() | run_one() usava ATR×2.0/1.5 per S16; run_adaptive() usava 3.0/1.2 → risultati individuali non confrontabili | Allineato run_one() a 3.0/1.2 come in STRATEGY_PARAMS |
| 2026-04-16 | Backtest combinato Elite 4 + RM su MT5 reale | Eseguito strategy-engine-v2.py --file xauusd_h1_mt5.json --rm e --file xauusd_m30_mt5.json --rm | M30 superiore: PF 1.203 vs 1.084 H1; P&L +$4915 vs +$2241; DD $960 vs $2529. Con RM M30: +$6150 PF 1.202 +$12.66/gg. Raccomandazione: M30 come TF primario |
