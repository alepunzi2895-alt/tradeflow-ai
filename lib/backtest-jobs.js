...
// Schema una sola volta per istanza serverless: prima girava a OGNI richiesta, compreso il
// poll del worker ogni 15s (CREATE + 2 ALTER falliti + scansioni complete senza indice) — una
// delle cause del blocco letture Turso del 2026-09-25 (piano esaurito, vedi 06_known_issues.md).
...
function setup(db) {
  if (!schemaReady) schemaReady = (async () => {
    ...
    // Il claim filtra per status e ordina per created_at: con l'indice legge solo i job
    // queued/running invece dell'intera storia (risultati compresi).
    await db.execute('CREATE INDEX IF NOT EXISTS idx_backtest_jobs_status ON backtest_jobs(status, created_at)');
    await db.execute('CREATE INDEX IF NOT EXISTS idx_backtest_jobs_user ON backtest_jobs(user_id, status)');
...
export async function workerStatusPush(db,b) {
  const errors=b.errors&&typeof b.errors==='object'?Object.fromEntries(Object.entries(b.errors).slice(0,10).map(([k,v])=>[String(k).slice(0,40),{at:String(v?.at||'').slice(0,40),error:String(v?.error||'').slice(0,300)}])):{};
  const payload=JSON.stringify({seen_at:new Date().toISOString(),host:String(b.host||'').slice(0,60),python:String(b.python||'').slice(0,20),errors,
