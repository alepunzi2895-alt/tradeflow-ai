...
let readersReady = null;   // una volta per istanza, non a ogni mt5_get (letture Turso)
export async function ensureSystemReaders(db) {
  if (!readersReady) readersReady = db.execute('CREATE TABLE IF NOT EXISTS system_readers (user_id TEXT PRIMARY KEY, created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)')
    .catch(e => { readersReady = null; throw e; });
  return readersReady;
}
